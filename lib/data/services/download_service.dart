...
class DownloadService {
  ...
  static Future<bool> downloadPDF({required String url, required String fileName}) async {
    printX("Download PDF Service call $url");
    String accessToken = Get.find<ApiClient>().sharedPreferences.getString(SharedPreferenceHelper.accessTokenKey) ?? "";
    Dio dio = Dio();
    ...
    try {
      var data = await dio.download(
        ...
        filePath,
        options: Options(
          headers: {"Authorization": "Bearer $accessToken", "dev-token": Environment.devToken, "Accept": "application/pdf"},
        ),
