...
class ChatBubbleClipper10 extends CustomClipper<Path> {
  ...
  ChatBubbleClipper10({this.type, this.radius = 15, this.nipSize = 8, this.sizeRatio = 3});
  ...
  Path getClip(Size size) {
    ...
    if (type == BubbleType.sendBubble) {
      ...
      path.quadraticBezierTo(size.width - nipSize, nipSize, size.width - nipSize, sizeRatio * nipSize);

      path.lineTo(size.width - nipSize, size.height - radius);
      path.arcToPoint(Offset(size.width - nipSize - radius, size.height), radius: Radius.circular(radius));
      ...
      path.arcToPoint(Offset(nipSize, size.height - radius), radius: Radius.circular(radius));
