...
class SocialAuthSection extends StatefulWidget {
  ...
  const SocialAuthSection({super.key, this.googleAuthTitle = MyStrings.google, this.appleAuthTitle = MyStrings.apple});
    ...
    return GetBuilder<SocialLoginController>(builder: (controller) {
      return Column(
        children: [
          Row(
            children: [
              if (controller.repo.apiClient.isGoogleLoginEnabled() == true) ...[
                Expanded(
                  child: InkWell(
                    onTap: () {
                    ...
                    customBorder: RoundedRectangleBorder(borderRadius: BorderRadius.circular(Dimensions.mediumRadius)),
              ...
              if (controller.repo.apiClient.isAppleLoginEnabled() == true && Platform.isIOS) ...[
                Expanded(
                  child: InkWell(
                    onTap: () {
                    ...
                    child: Container(
                      ...
                      child: Row(
                        ...
                        crossAxisAlignment: CrossAxisAlignment.center,
                        children: [
                          controller.isAppleSignInLoading ? SizedBox(height: 15, width: 15, child: CircularProgressIndicator(color: MyColor.primaryColor)) : MyLocalImageWidget(imagePath: MyImages.apple, height: 25, width: 25, boxFit: BoxFit.contain),
                          SizedBox(width: Dimensions.space10),
                          Text(MyStrings.apple.tr, style: regularDefault.copyWith(fontWeight: FontWeight.w500, fontSize: 16)),
                        ],
                      ),
                    ),
                  ),
          ...
          if (controller.repo.apiClient.isGoogleLoginEnabled() == true || controller.repo.apiClient.isAppleLoginEnabled() == true) ...[
